...
namespace App\Http\Controllers\Api\V1\Inventory\Concerns;
 ...
 *
 * An exists rule on the table alone accepts another organization's id: the
 * request then either links that row into this organization's document or
 * learns that the id exists. These rules accept only a row of the caller's
 * organization. Users carry no tenant scope, so a user id is checked through
 * ownedBy('users') like any other table.
 */
trait ValidatesOwnedRows
{
    ...
    /**
     * An exists rule for a table without an organization column, such as
     * product variants or warehouse locations: the row is accepted only when
     * the parent it points to through $foreignKey belongs to the caller's
     * organization.
     */
